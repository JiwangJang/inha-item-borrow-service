...
class RequestControllerTest {
    ...
    void setUp() {
        ...
        testUser = new User(
                TEST_USER_ID, "password", "email@test.com", "테스트유저", "010-1234-5678", "refreshToken", authorities
        ) {
    ...
    void evaluateRequestSuccess() throws Exception {
        ...
        mockMvc.perform(patch("/requests/{request-id}/evaluate", 1)
                        .with(csrf())
                        .with(user(TEST_USER_ID).authorities(new SimpleGrantedAuthority("ROLE_DIVISION_MEMBER")))
                        .contentType(MediaType.APPLICATION_JSON)

...
public class SignUpRequestSessionCache {
    public ConcurrentHashMap<String, SignUpRequestSession> cache = new ConcurrentHashMap<>();
     ...
     */
    public Optional<SignUpRequestSession> get(String id) {
    ...
    public void set(String id) {
        cache.put(id, new SignUpRequestSession());
    }
    ...
    /**
     * 세션연장 메서드
     * 사용자가 핸드폰 인증을 요청하거나 비밀번호 인증을 완료했을때 실행될 메서드
     * 
     * @param id 대상 대여자 아아디
     */
    public void extendTtl(String id) {
        ...
        cache.computeIfPresent(id, (k, v) -> {
            v.setTtl(System.currentTimeMillis() + 300000);
            return v;
        });
    }
    ...
    public boolean isAllPassed(String id) {
        SignUpRequestSession session = cache.get(id);
        if (session == null) {
